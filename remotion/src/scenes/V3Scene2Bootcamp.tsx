...
export const V3Scene2Bootcamp = () => {
  ...
  const textOpacity = interpolate(frame, [20, 45], [0, 1], { extrapolateRight: "clamp", extrapolateLeft: "clamp" });
  ...
  return (
    <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
      {/* Bootcamp image */}
      <div style={{
        width: 1100, height: 800, borderRadius: 30, overflow: "hidden",
        transform: `scale(${imgScale})`,
        boxShadow: "0 30px 80px rgba(0,0,0,0.4)",
      }}>
        <Img src={staticFile("images/v3-scene2-bootcamp.jpg")} style={{ width: "100%", height: "100%", objectFit: "cover", transform: `scale(${imgZoom})` }} />
      </div>

      {/* Date badges floating */}
      {dates.map((date, i) => {
        const s = spring({ frame: frame - 30 - i * 8, fps, config: { damping: 10 } });
        const positions = [
          { left: 80, top: 100 }, { right: 80, top: 140 },
          { left: 100, bottom: 120 }, { right: 100, bottom: 160 },
        ];
        return (
          <div key={i} style={{
